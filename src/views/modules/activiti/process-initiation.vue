...
export default {
  ...
  data () {
    return {
      mixinViewModuleOptions: {
        getDataListURL: '/activiti/process/lastestPage',
  ...
  methods: {
    ...
    createProcessInstance (row) {
      this.$http.get(`/activiti/process/getProcDefBizRoute/${row.id}`).then(({ data: res }) => {
        if (res.code !== 0) {
    ...
    startProcess (key) {
      ...
      this.$http.post(`/activiti/running/start`, params).then(({ data: res }) => {
        if (res.code !== 0) {
          return this.$message.error(res.msg)
        }
        this.$message({
