	...
	<el-dialog :visible.sync="visible" :title="!dataForm.id ? $t('add') : $t('update')" :close-on-click-modal="false"
	 :close-on-press-escape="false">
		<el-form :model="dataForm" :rules="dataRule" ref="dataForm" @keyup.enter.native="dataFormSubmitHandle()" label-width="120px">
			...
			<el-form-item prop="mbdLightroads" label="设备路数">
				<el-select filterable v-model="dataForm.mbdLightroads" placeholder="请选择" clearable style="width: 100%;">
					<el-option v-for="item in eqNumberList" :key="item.dictValue" :label="item.dictLabel" :value="item.dictValue">
					</el-option>
				</el-select>
			</el-form-item>
		...
		methods: {
			...
			getInfo() {
				this.$http.get(`/system/scmodbusdevicedc/${this.dataForm.id}`).then(({
					...
					this.dataForm.mbdLightroads = String(this.dataForm.mbdLightroads)
				}).catch(() => {})
			},
			// 表单提交
