		 ...
		 style="width: 100%" :header-cell-style="{fontSize:'16px'}">
			...
			<el-table-column prop="type" align="center" label="位置" width="200">
				<template slot-scope="scope">
					...
					<span v-else-if="scope.row.type == 3">宿舍房间</span>
					<span v-else-if="scope.row.type == 4">UWB</span>
				</template>
			</el-table-column>
			<el-table-column align="center" label="异常次数" width="180">
				<template slot-scope="scope">
					{{scope.row.dataAnalysisInfoDTOList.length}}
				</template>
			</el-table-column>
			<el-table-column align="center">
				...
				<template slot-scope="scope">
					<div class="squareContent" style="display: flex;flex-wrap:wrap">
						<el-tooltip v-for="(item,index) in dateRange" effect="light" :content="item" placement="top-start" :key="index">
							<div class="divSquare" :style="{backgroundColor:scope.row.dataAnalysisInfoDTOList.includes(item) ?'red' : 'green'}">
							</div>
						</el-tooltip>
					</div>
				</template>
	...
	export default {
		...
		data() {
			return {
				...
				daterange: [new Date(new Date().getTime() - 1296000000), new Date(new Date() - 86400000)], // 一个月的时间戳
		...
		methods: {
			viewHandle(row) {
				...
				this.$nextTick(() => {
					this.$refs.viewRef.init(row, this.dataForm, "/system/dataAnalysis/notInOrOut")
				})
			},
			// 列表查询
			getDataList() {
				...
				this.loading = true
				this.$http.get("/system/dataAnalysis/notInOrOut", {
					...
					if (res.data.length > 0) {
						...
						this.dataList.forEach((item, index, array) => {
							item.dataAnalysisInfoDTOList = item.dataAnalysisInfoDTOList.map(item => {
