<template>
	<el-card shadow="never" class="aui-card--fill staff-report">
		...
		<el-table border ref="table" :data="dataList" style="width: 100%" :span-method="objectSpanMethod" :header-cell-style="{fontSize:'18px'}">
			<el-table-column :label="dataForm.dateType==1 ? '教职工被访周报表' : '教职工被访月报表'" align="center">
				<el-table-column prop="dateStr" align="center" label="时间段">
				</el-table-column>
				<el-table-column prop="name" align="center" label="教职工姓名">
				</el-table-column>
				<el-table-column prop="count" align="center" label="被访次数">
				</el-table-column>
			</el-table-column>
		</el-table>
	</el-card>
...
<script>
	...
	export default {
		...
		methods: {
			// 列表查询
			getDataList() {
				this.$http.get("/system/reportForm/visitorWorkersWeekMonth", {
					params: this.dataForm
				}).then(({
					data: res
				}) => {
					this.dataList = res.data
					buildMerge(this.dataList, 'dateStr')
				})
			...
			exportHandle() {
				...
				window.location.href = `${window.SITE_CONFIG['apiURL']}/system/reportForm/visitorWorkersWeekMonthExport?${params}`
			},
			// 周月切换
			dateTypeHandle() {
				this.getDataList()
