	...
	<el-card shadow="never" class="aui-card--fill academic-report">
		<el-form :model="dataForm" :inline="true" @keyup.enter.native="getDataList()" ref="ruleForm">
			...
			<el-form-item>
				...
				<el-button type="success" @click="exportHandleClick()">导出</el-button>
			</el-form-item>
		</el-form>
	...
	export default {
		...
		data() {
			return {
				mixinViewModuleOptions: {
					getDataListURL: '/system/reportForm/academicStatistics',
		...
		methods: {
			exportHandleClick() {
				...
				window.location.href = `${window.SITE_CONFIG['apiURL']}/system/reportForm/exportgetByAcademicWeek?${params}`
			},
			numTypeHandle(e) {
				this.dateType = e
				this.query()
