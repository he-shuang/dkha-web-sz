	...
	<el-card shadow="always" class="aui-card--fill">
		...
		<el-table :data="dataList">
			...
			<el-table-column align="center" prop="temperature" label="温度值℃">
				<template slot-scope="scope">
					<span style="color:red" v-if="scope.row.temperature >= $getDictLabel('thermalImaging','0' )">{{(scope.row.temperature*1).toFixed(2)}}</span>
					<span v-else-if="scope.row.temperature < $getDictLabel('thermalImaging','1' )">{{(scope.row.temperature*1).toFixed(2)}}</span>
		...
		data() {
			return {
				mixinViewModuleOptions: {
					getDataListURL: '/system/scthermalimaging/thermalList/1',
