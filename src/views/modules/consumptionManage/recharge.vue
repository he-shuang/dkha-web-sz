	...
	<el-card shadow="never" class="aui-card--fill">
		...
		<el-table v-loading="dataListLoading" :data="dataList">
			...
			<el-table-column label="充值金额" align="center" show-overflow-tooltip>
				<template slot-scope="scope">{{(scope.row.rechargeAmt * 1).toFixed(2)}}</template>
			</el-table-column>
			<el-table-column label="充值赠送金额" align="center" show-overflow-tooltip>
				<template slot-scope="scope">{{(scope.row.giveAmt * 1).toFixed(2)}}</template>
			</el-table-column>
			<el-table-column label="充值总金额" align="center" show-overflow-tooltip>
				<template slot-scope="scope">{{(scope.row.rechargeTotalAmt * 1).toFixed(2)}}</template>
			</el-table-column>
			<el-table-column prop="createDate" label="充值时间" align="center" show-overflow-tooltip>
			</el-table-column>
			<el-table-column prop="operName" label="操作人" align="center" show-overflow-tooltip>
			</el-table-column>
		</el-table>
		...
		data() {
			return {
				mixinViewModuleOptions: {
					getDataListURL: '/system/scconsumptionsystemrecharge/page',
